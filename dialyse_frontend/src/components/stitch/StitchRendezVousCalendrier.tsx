                  ...
                  <div key={jour}
                    ...
                    <div className="space-y-0.5 overflow-hidden">
                      {rdvJour.slice(0,3).map(rdv => (
                        <div key={rdv.id} className={`${TYPE_STYLES[rdv.type].bg} border-l-4 ${TYPE_STYLES[rdv.type].border} rounded-r-lg px-2 py-1 overflow-hidden`}>
                          <p className={`text-[10px] font-bold ${TYPE_STYLES[rdv.type].text} truncate`}>{rdv.heure}</p>
                          <p className={`text-[11px] font-semibold ${TYPE_STYLES[rdv.type].text} truncate`}>{rdv.patientPrenom} {rdv.patientNom}</p>
                        </div>
                      ))}
