      ...
      <table className="w-full text-left">
        ...
        <tfoot>
          <tr className="border-t-2 border-slate-200 bg-slate-50">
            <td colSpan={3} className="px-4 py-3 text-xs font-bold text-slate-600 text-right uppercase tracking-wide">
              Total
            </td>
...
function SectionCard({
  icon, title, sub, iconBg, children, locked, lockMsg,
}: {
  icon: string; title: string; sub: string; iconBg: string;
  children: React.ReactNode; locked?: boolean; lockMsg?: string;
}) {
  return (
    <div className="relative">
      {locked && (
        <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-white/60 backdrop-blur-[2px]">
          <div className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2.5 rounded-xl text-xs font-bold shadow-xl">
...
function VerificationKitInner() {
  ...
  /* Info patient commune */
  const [patient, setPatient] = useState({ nom: '', age: '', sexe: 'M', adresse: '', service: '', date: '' });
    ...
    <div className="min-h-screen bg-slate-50/60 p-6 space-y-6 max-w-5xl mx-auto">
      ...
      >
        ...
        <span className={`ml-auto px-3 py-1 rounded-full text-[10px] font-bold border ${
          ...
          {isPremiereSeance ? '🔵 1ère séance active' : '🟢 Séances suivantes'}
        </span>
      </motion.div>
      ...
      {/* ── Kit 1ère séance — masqué si séance > 1 ── */}
      {isFirstSeance && (
      <SectionCard
        icon="vaccines" title="Kit Hémodialyse — 1ère Séance"
        sub="Ordonnance complète · Nouveau patient"
        iconBg="bg-gradient-to-r from-blue-50 to-blue-100/50"
        locked={false}
        lockMsg=""
      >
        <KitTable items={KIT_PREMIERE} rows={rows1} onChange={(i, f, v) => updateRow(setRows1, i, f, v)} />
        <div className="px-5 py-4 border-t border-slate-100 flex justify-end gap-2 no-print">
          <button onClick={() => setRows1(KIT_PREMIERE.map(i => ({ ...i })))} className="px-4 py-2 text-xs font-semibold text-slate-500 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
            Réinitialiser
          </button>
          <button onClick={handlePrint} className="px-4 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-colors">
            <span className="material-symbols-outlined text-sm align-middle mr-1">print</span>Imprimer
          </button>
          <button className="px-4 py-2 text-xs font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors">
      ...
      {!isFirstSeance && (
      <SectionCard
        icon="autorenew" title="Kit Hémodialyse — Séances Suivantes"
        sub="Ordonnance renouvellement"
        iconBg="bg-gradient-to-r from-emerald-50 to-emerald-100/50"
        locked={false}
        lockMsg=""
      >
        <KitTable items={KIT_SUIVANTE} rows={rows2} onChange={(i, f, v) => updateRow(setRows2, i, f, v)} />
        <div className="px-5 py-4 border-t border-slate-100 flex justify-end gap-2 no-print">
          <button onClick={handlePrint} className="px-4 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-colors">
            <span className="material-symbols-outlined text-sm align-middle mr-1">print</span>Imprimer
          </button>
          <button className="px-4 py-2 text-xs font-bold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 transition-colors">
            <span className="material-symbols-outlined text-sm align-middle mr-1">save</span>Enregistrer
          </button>
        </div>
      </SectionCard>
      ...
      {isFirstSeance && (
      <SectionCard
        icon="medical_services" title="Ordonnance Kit 1er Soin — Service Hémodialyse"
        sub="Matériel de soin cathéter"
        iconBg="bg-gradient-to-r from-amber-50 to-amber-100/50"
        locked={false}
        lockMsg=""
      >
        <KitTable items={KIT_SOIN} rows={rowsSoin} onChange={(i, f, v) => updateRow(setRowsSoin, i, f, v)} />
        <div className="px-5 py-4 border-t border-slate-100 flex justify-end gap-2 no-print">
          <button onClick={handlePrint} className="px-4 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-colors">
            <span className="material-symbols-outlined text-sm align-middle mr-1">print</span>Imprimer
          </button>
          <button className="px-4 py-2 text-xs font-bold text-white bg-amber-600 rounded-xl hover:bg-amber-700 transition-colors">
            <span className="material-symbols-outlined text-sm align-middle mr-1">save</span>Enregistrer
          </button>
        </div>
      </SectionCard>
