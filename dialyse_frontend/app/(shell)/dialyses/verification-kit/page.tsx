...
/* ─── Carte "Vu" — affichée quand section fermée ─── */
function VuCard({ title, icon, onReopen }: {
  title: string; icon: string; onReopen: () => void;
}) {
  return (
    <motion.div
...
/* ─── Section Kit avec logique Vu ─── */
function KitSection({
  icon, title, sub, iconBg,
  rows, onChangeRow, onReset,
  status, onVu,
  resetKit,
  color = 'blue',
}: {
  ...
  // Quand status devient 'vu', fermer automatiquement
  useEffect(() => {
    if (status === 'vu') setOpen(false);
  }, [status]);

  const colorMap = {
    blue:    { btn: 'bg-blue-600 hover:bg-blue-700',       vuBg: 'bg-blue-50',    vuBorder: 'border-blue-200'    },
    emerald: { btn: 'bg-emerald-600 hover:bg-emerald-700', vuBg: 'bg-emerald-50', vuBorder: 'border-emerald-200' },
    amber:   { btn: 'bg-amber-600 hover:bg-amber-700',     vuBg: 'bg-amber-50',   vuBorder: 'border-amber-200'   },
  }[color];

  // ── Section fermée → afficher VuCard ──
  if (status === 'vu' && !open) {
    return (
      <VuCard
        title={title}
            ...
            <div className="px-5 py-4 border-t border-slate-100 flex items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className={`px-5 py-2 text-xs font-black text-white rounded-xl transition-all cursor-pointer flex items-center gap-1.5 shadow-sm disabled:opacity-60 ${colorMap.btn}`}
                >
                  {status === 'saving' ? (
                    <>
...
function VerificationKitInner() {
  ...
  // ✅ Statuts séparés pour chaque section
  ...
  // ✅ Fonction Vu — sauvegarde BD + fermeture
  const handleVu = async (
    ...
    if (!patientIdParam) {
      showToast('⚠️ Aucun patient — revenez depuis le tableau de bord', 'error');
      return;
    }
    ...
    try {
      ...
      if (allOk) {
        // ✅ Marquer VU → déclenche fermeture automatique dans KitSection
        setStatus('vu');
        showToast(`✅ Kit "${kitLabel}" validé et enregistré`, 'success');
      } else {
        throw new Error('Erreur serveur');
      }
    } catch {
      setStatus('idle');
      showToast('❌ Erreur enregistrement — réessayez', 'error');
    }
  };
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        ...
        <span className={`text-xs font-semibold ${patientIdParam ? 'text-blue-700' : 'text-amber-700'}`}>
          {patientIdParam
            ...
            : 'Aucun patient lié — revenez depuis le tableau de bord pour enregistrer'
          }
        </span>
      </motion.div>

      {/* ── Toggle type de séance ── */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
          {(['premiere', 'suivante'] as SeanceType[]).map(t => (
            <button key={t} onClick={() => setSeanceType(t)}
